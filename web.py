...
CONTEXT_ARTICLES = 2      # статей в контексте. МАЛО намеренно: на 2 ядрах обработка
                          # промпта — главный тормоз (~40-80с), режем объём. Нужная норма
                          # — в лид-чанке топ-статьи, которую reorder ставит первой.
CONTEXT_CHARS = 650       # символов на статью (ключевая норма — в начале статьи)
